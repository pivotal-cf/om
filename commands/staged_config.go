...
func (ec StagedConfig) chooseCredentialHandler() configparser.CredentialHandler {
	if ec.Options.IncludePlaceholder {
		return configparser.WithPlaceholderHandleCredentialFunc()
	}

	if ec.Options.IncludeCredentials {
		return configparser.WithCredentialHandleCredentialFunc(ec.service)
	}

	return configparser.DefaultHandleCredentialFunc()
}
