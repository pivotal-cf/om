...
This authenticated command lists all products and will display whether they are
unchanged (no pending changes) or changed (has pending changes).

## Command Usage
```
Usage:
  om [OPTIONS] pending-changes [pending-changes-OPTIONS]
...
[pending-changes command options]
          --check              Exit 1 if there are any pending changes. Useful
                               for validating that Ops Manager is in a clean
                               state.
